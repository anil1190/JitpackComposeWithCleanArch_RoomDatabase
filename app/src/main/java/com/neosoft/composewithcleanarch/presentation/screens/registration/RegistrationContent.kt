...
@Composable
fun RegistrationContent(viewModel:RegistrationViewModel= hiltViewModel()){
    ...
    val emp_id   by viewModel.emp_id.collectAsStateWithLifecycle()
    val areInputsValid by viewModel.areInputsValid.collectAsStateWithLifecycle()

    val passwordFocusRequester = remember { FocusRequester() }
    val nameFocusRequester = remember { FocusRequester() }
    val empIdFocusRequester = remember { FocusRequester() }
            ...
            when (event) {
                ...
                is ScreenEvent.RequestFocus -> {
                    when (event.textFieldKey) {
                        ...
                        FocusedTextFieldKey.EMP_ID -> empIdFocusRequester.requestFocus()
                        else -> {}
                    }
                }
    ...
    Column(
        ...
        CustomTextField(
            modifier = Modifier
                .focusRequester(empIdFocusRequester)
                .onFocusChanged { focusState ->
                    viewModel.onTextFieldFocusChanged(
                        key = FocusedTextFieldKey.EMP_ID,
                        isFocused = focusState.isFocused
                    )
                },
            ...
            inputWrapper = emp_id,
            onValueChange = viewModel::onEmpIdEntered,
            onImeKeyAction = viewModel::onContinueClick
        )
